# Hola, soy Alejandro Padilla 👋

Soy estudiante de Computación en la Universidad Nacional de Loja, enfocado en desarrollo de software, fundamentos de backend y construcción de proyectos web.

Actualmente estoy fortaleciendo mis bases en Java, estructuras de datos, SQL, Git, Docker, TypeScript y desarrollo full-stack. Busco una oportunidad como practicante de programación en modalidad remota, donde pueda aprender en un entorno real, aportar con responsabilidad y seguir creciendo como desarrollador.

## Sobre mí

- 🎓 Estudiante de Computación — Universidad Nacional de Loja
- 💻 Interés principal: desarrollo backend y full-stack
- 🌎 Ubicación: Ecuador
- 🚀 Buscando práctica remota en programación
- 📚 En aprendizaje constante sobre buenas prácticas, arquitectura y desarrollo web moderno

## Tecnologías y herramientas

### Lenguajes

![Java](https://img.shields.io/badge/Java-21-orange?style=for-the-badge&logo=openjdk&logoColor=white)
![SQL](https://img.shields.io/badge/SQL-PostgreSQL-blue?style=for-the-badge&logo=postgresql&logoColor=white)
![TypeScript](https://img.shields.io/badge/TypeScript-Learning-3178C6?style=for-the-badge&logo=typescript&logoColor=white)

### Backend y bases de datos

![Maven](https://img.shields.io/badge/Maven-Build_Tool-C71A36?style=for-the-badge&logo=apachemaven&logoColor=white)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-Learning-4169E1?style=for-the-badge&logo=postgresql&logoColor=white)
![Prisma](https://img.shields.io/badge/Prisma-Learning-2D3748?style=for-the-badge&logo=prisma&logoColor=white)

### Frontend y web

![HTML5](https://img.shields.io/badge/HTML5-Web-E34F26?style=for-the-badge&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-Web-1572B6?style=for-the-badge&logo=css3&logoColor=white)
![React](https://img.shields.io/badge/React-Learning-61DAFB?style=for-the-badge&logo=react&logoColor=black)
![Next.js](https://img.shields.io/badge/Next.js-Learning-000000?style=for-the-badge&logo=nextdotjs&logoColor=white)

### Herramientas

![Git](https://img.shields.io/badge/Git-Version_Control-F05032?style=for-the-badge&logo=git&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-Portfolio-181717?style=for-the-badge&logo=github&logoColor=white)
![Linux](https://img.shields.io/badge/Linux-Daily_Use-FCC624?style=for-the-badge&logo=linux&logoColor=black)
![Docker](https://img.shields.io/badge/Docker-Learning-2496ED?style=for-the-badge&logo=docker&logoColor=white)

## Proyecto destacado

### Agenda e Inventarios Inteligentes

Proyecto académico en Java para gestionar citas médicas, pacientes e inventario hospitalario usando estructuras de datos, algoritmos de búsqueda, algoritmos de ordenamiento y datasets CSV.

**Tecnologías:** Java 21, Maven, Apache Commons CSV, JUnit 5

**Conceptos aplicados:**

- Programación orientada a objetos
- Arreglos y listas simplemente enlazadas
- Bubble Sort, Selection Sort e Insertion Sort
- Búsqueda secuencial, búsqueda con centinela y búsqueda binaria
- Lectura de datasets CSV
- Análisis de casos borde

🔗 Repositorio: [AgendaInventarioInteligentes](https://github.com/AlejandroTatum/AgendaInventarioInteligentes)

## Objetivo actual

Estoy construyendo un portafolio más sólido con proyectos pequeños pero bien terminados, documentados y desplegados. Mi objetivo es conseguir una práctica remota donde pueda integrarme a un equipo, recibir feedback técnico y aportar desde mis fundamentos.

## Contacto

- 📧 Email: [alejandro.padilla@unl.edu.ec](mailto:alejandro.padilla@unl.edu.ec)
- 💼 LinkedIn: [Alejandro Padilla](https://www.linkedin.com/in/alejandro-padilla-717abb37b)
- 🐙 GitHub: [AlejandroTatum](https://github.com/AlejandroTatum)
